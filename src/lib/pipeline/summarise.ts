...
export function buildUserPrompt(transcript: string): string {
  return [
    ...
    "4. EPISODE SPONSORSHIPS — Every distinct sponsor or advertiser read in the episode (mid-roll, pre-roll, host-read ad spots, 'brought to you by', discount codes, etc.). For each: sponsor or product name, one short line on what they offer, and include a markdown link to the specific landing page or offer URL if the host gives one (format: [Product or offer](https://example.com/path)). If a URL is spoken but incomplete, omit the link and note 'URL not given in full'. If there were no sponsor segments, write exactly: No sponsorships in this episode. Start this section with the exact heading line: 4. EPISODE SPONSORSHIPS (so it can be parsed reliably). After that single heading line, write sponsor lines in plain sentences only: no second heading, no # or ## lines, and no **bold** markers (the email template adds its own title and styling).",
    "",
